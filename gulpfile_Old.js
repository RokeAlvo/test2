...
const bemModSeparator=nth.config.bemModSeparator;
...
function copyImg(cb) {
  let copiedImages = [];
  nth.blocksFromHtml.forEach(function(blockName) {
    
    let block=getBlockDir(blockName);
...
function writeSassImportsFile(cb) {
  ...
  // добавляем css правила для блоков, элементов и модификаторов на странице
  // getDirectories() возвращает путь относительно dir.blocks и  имя файла
  // TODO вынести разбор fileName  в функцию getDirectories()
  let allBlocksWithScssFiles = getDirectories('scss');
  allBlocksWithScssFiles.forEach(function(fileName){
    let blockName = fileName.slice(0, fileName.lastIndexOf('/')).replace(/\//g,'');
    let url = `${dir.blocks}${fileName}`;
  ...
  if (diff.length) {
    ...
    console.log('---------- Write new style.scss \nсодержание  styleImports: '+styleImports);
...
function writeJsRequiresFile(cb) {
  ...
  allBlocksWithJsFiles.forEach(function(blockName){
    ...
    jsRequiresList.push(`../blocks/${blockName}`)
  });
  allBlocksWithJsFiles.forEach(function(fileName){
    let blockName = fileName.slice(0, fileName.lastIndexOf('/')).replace(/\//g,'');
    let src = `../blocks/${fileName}`;
    if (nth.blocksFromHtml.indexOf(blockName) == -1) return;
...
function getClassesToBlocksList(file, enc, cb) {
  ...
  if (processThisFile) {
    ...
    for (let item of classesInFile) {
      ...
      if (nth.blocksFromHtml.indexOf(item) + 1) continue;
...
/**
 * Получение директории блока, элемента или модификатора по имени 
 * @param {string} blockName имя блока
 * @return {string}    директория блока относительно папки блоков
 */
function getBlockDir(blockName) {
  // в общем виде blockName: blockName+[__elementName]+[_modName]+[_modValue]
  // разбираем blockName
  // из конфига получаем разделители, по умолчанию '--' 
  let reg=/(?=__)|(?=--)/;
  if(bemModSeparator=='_')  {reg=/(?<!_)(?=_)/};
  return blockName.replace(reg, '/');
}